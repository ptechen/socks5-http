use {
    error::{Error, Result},
    socks5_protocol::{Address, AddressType, AsyncStreamOperation, AuthMethod, Command, Reply, StreamOperation, UserKey, Version},
};
    ...
    Self: Send,
{
    ...
    fn read_method(&mut self) -> impl Future<Output = Result<AuthMethod>> + Send {
        async move {
            let value = AuthMethod::from(self.read_u8().await?);
            match value {
                AuthMethod::NoAuth | AuthMethod::UserPass => Ok(value),
                _ => Err(Error::InvalidAuthMethod(value.to_string())),
            }
        }
    }

    fn read_command(&mut self) -> impl Future<Output = Result<Command>> + Send {
        async move {
    ...
    fn read_atyp(&mut self) -> impl Future<Output = Result<AddressType>> + Send {
        async move {
            let value = self.read_u8().await?;
    ...
    fn read_reserved(&mut self) -> impl Future<Output = Result<()>> + Send {
        async move {
            let value = self.read_u8().await?;
            match value {
                0x00 => Ok(()),
    ...
    fn read_fragment_id(&mut self) -> impl Future<Output = Result<()>> + Send {
        async move {
            let value = self.read_u8().await?;
            if value == 0x00 {
                Ok(())
    ...
    fn read_address(&mut self) -> impl Future<Output = Result<Address>> + Send {
        Address::retrieve_from_async_stream(self)
    }

    fn read_string(&mut self) -> impl Future<Output = Result<String>> + Send {
        async move {
    ...
    fn read_auth_version(&mut self) -> impl Future<Output = Result<()>> + Send {
        async move {
            let value = self.read_u8().await?;
            if value != 0x01 {
    ...
    fn read_auth_status(&mut self) -> impl Future<Output = Result<()>> + Send {
        async move {
            let value = self.read_u8().await?;
            if value != 0x00 {
    ...
    fn read_selection_msg(&mut self) -> impl Future<Output = Result<AuthMethod>> + Send {
        async move {
    ...
    fn read_final(&mut self) -> impl Future<Output = Result<Address>> + Send {
        async move {
            self.read_version().await?;
...
impl<T: AsyncReadExt + Unpin + Send> Socks5Reader for T {}

pub trait Socks5Writer: AsyncWriteExt + Unpin
where
    Self: Send,
{
    fn write_version(&mut self) -> impl Future<Output = Result<()>> + Send {
        async {
    ...
    fn write_method(&mut self, method: AuthMethod) -> impl Future<Output = Result<()>> + Send {
        async move {
    ...
    fn write_command(&mut self, command: Command) -> impl Future<Output = Result<()>> + Send {
        async move {
    ...
    fn write_atyp(&mut self, atyp: AddressType) -> impl Future<Output = Result<()>> + Send {
        async move {
    ...
    fn write_reserved(&mut self) -> impl Future<Output = Result<()>> + Send {
        async {
    ...
    fn write_auth_version(&mut self) -> impl Future<Output = Result<()>> + Send {
        async move {
    ...
    fn write_methods(&mut self, methods: &[AuthMethod]) -> impl Future<Output = Result<()>> + Send {
        async move {
    ...
    fn write_selection_msg(&mut self, methods: &[AuthMethod]) -> impl Future<Output = Result<()>> + Send {
        async {
    ...
    fn write_final(&mut self, command: Command, addr: &Address) -> impl Future<Output = Result<()>> + Send {
        async move {
            self.write_version().await?;
...
impl<T: AsyncWriteExt + Unpin + Send> Socks5Writer for T {}
    ...
    A: Into<Address>,
{
    ...
    match method {
        ...
        _ => return Err(Error::InvalidAuthMethod(method.to_string())),
    }
...
/// socks5_client::connect(&mut stream, ("google.com", 80), None).await?;
///
/// # Ok(())
/// # }
/// ```
...
/// # #[tokio::main(flavor = "current_thread")]
/// # async fn main() -> Result<()> {
/// use socks5_client::SocksListener;
/// use tokio::{io::BufStream, net::TcpStream};
///
/// let stream = TcpStream::connect("my-proxy-server.com:54321").await?;
/// let mut stream = BufStream::new(stream);
    ...
    S: AsyncWriteExt + AsyncReadExt + Send + Unpin,
{
    ...
    /// Parses the udp-based socks5_server response packet, the format is same as the client request packet.
...
pub trait UdpClientTrait {
    fn send_to<A>(&mut self, buf: &[u8], addr: A) -> impl Future<Output = Result<usize>> + Send
    where
        A: Into<Address> + Send + Unpin;

    fn recv_from(&mut self, timeout: Duration, buf: &mut Vec<u8>) -> impl Future<Output = Result<(usize, Address)>> + Send;
}
...
mod tests {
    ...
    async fn connect(addr: &str, auth: Option<UserKey>) {
        ...
        crate::connect(&mut socket, Address::from(("baidu.com", 80)), auth).await.unwrap();
    }
    ...
    impl UdpTest<SocksUdpClient> {
        async fn datagram() -> Self {
            ...
            let client = crate::create_udp_client(addr, None).await.unwrap();
