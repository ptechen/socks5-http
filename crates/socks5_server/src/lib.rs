...
/// The socks5 socks5_server itself.
///
/// The socks5_server can be constructed on a given socket address, or be created on an existing TcpListener.
///
...
impl<O: 'static> Server<O> {
    /// Create a new socks5 socks5_server with the given TCP listener and authentication method.
    #[inline]
    pub fn new(listener: TcpListener, auth: AuthAdaptor<O>) -> Self {
        Self { listener, auth }
    }

    /// Create a new socks5 socks5_server on the given socket address and authentication method.
    #[inline]
    ...
    /// Get the the local socket address binded to this socks5_server
    #[inline]
